...
#include <fstream>
#include <string>
#include "time_calculation.h"//time_cal class header
...
{
	//time_cal class usage
	time_cal timeCalculate;
	...
	for (int i = 1; i < argc; i++) {
		...
		if ((string(argv[i]).find("-") == 0 && string(argv[i]).find("o") != string::npos)) {
			...
			//time_cal class usage
			timeCalculate.time_cal_start();
			jacobi_method(A, n);
			timeCalculate.time_cal_end();
			timeCalculate.get_duration();
			system("pause");
			...
			sprintf_s(filename, "eigen1 n%d nametag%d.txt", n, nametag);
			...
			for (int j = 1; j < argc; j++) {
				if ((string(argv[j]).find("-") == 0 && string(argv[j]).find("l") != string::npos)) {
				...
				else if ((string(argv[j]).find("-") == 0 && string(argv[j]).find("a") != string::npos)) {
...
*/
		...
		//two particle problem
			...
			cout << "choose 0.01, 0.5, 1.0, 5.0" << endl;
			cin >> w_r;
			...
			sprintf_s(filename, "eigen n%d nametag%d.txt", n, nametag);
			fs.open(filename, ios::out);
			for (int i = 0; i < n; i++) {
				fs << diag_2[i] << endl;
			}
